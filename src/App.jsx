...
var AGENT_COMPUTE={
  ...
  priya:function(q){/* Boundary: AR_Aging + Denial_Logs + Payer_Mix only — no variance bridge */
    ...
    /* Payer Mix data */
    if(Object.keys(DB.payerMix).length>0){var payerAgg={};Object.values(DB.payerMix).forEach(function(clinicPayers){clinicPayers.forEach(function(pm){if(!payerAgg[pm.payer])payerAgg[pm.payer]={payer:pm.payer,grossCharges:0,adjustments:0,netRevenue:0,claims:0,denials:0};payerAgg[pm.payer].grossCharges+=pm.grossCharges;payerAgg[pm.payer].adjustments+=pm.adjustments;payerAgg[pm.payer].netRevenue+=pm.netRevenue;payerAgg[pm.payer].claims+=pm.claims;payerAgg[pm.payer].denials+=pm.denials;});});base.payerMixSummary=Object.values(payerAgg).map(function(p){return{payer:p.payer,grossCharges:Math.round(p.grossCharges/1000),netRevenue:Math.round(p.netRevenue/1000),claims:p.claims,denials:p.denials,denialRate:p.claims?Math.round(p.denials/p.claims*1000)/10:0};}).sort(function(a,b){return b.grossCharges-a.grossCharges;});}
    return base;
  },
  ...
  sam:function(q){/* Boundary: PE_Metrics, SSS, CAC only — receives executive summary (aggregated, not raw) */return FE.executiveSummary();},
  jordan:function(q){/* Boundary: Clinic_Health, Ops_Metrics + Provider_Productivity only */var lc=q.toLowerCase();if(lc.includes("comp")||lc.includes("doctor"))return FE.doctorCompImpact();
    var base=FE.clinicHealth();
    /* Enrich with provider productivity summary */
    if(Object.keys(DB.providerData).length>0){var provSum={totalProviders:0,avgUtil:0,avgCompRatio:0,totalRevenue:0,totalComp:0};Object.values(DB.providerData).forEach(function(provs){provs.forEach(function(p){provSum.totalProviders++;provSum.avgUtil+=p.utilization;provSum.avgCompRatio+=p.compRatio;provSum.totalRevenue+=p.revenue;provSum.totalComp+=p.comp;});});base.providerSummary={totalProviders:provSum.totalProviders,avgUtilization:Math.round(provSum.avgUtil/provSum.totalProviders*10)/10,avgCompRatio:Math.round(provSum.avgCompRatio/provSum.totalProviders*10)/10,totalRevenue:Math.round(provSum.totalRevenue/1000),totalComp:Math.round(provSum.totalComp/1000)};}
    return base;
  }
};
