...
var FE={
  ...
  appData:function(){
    ...
    return{portfolio_summary:portfolio_summary,regional_metrics:regional_metrics,clinic_details:clinic_details};
  }
};
/* ═══════════════════════════════════════════════════════════════════
...
var AGENTS=[
  ...
  {id:"alex",name:"Alex",title:"Forecasting & Scenarios",color:"#059669",data_access:["Forecast_Models","Monthly_Trend","Budget_Assumptions"],skills:["driver_based_forecasting","three_scenario_modeling","sensitivity_analysis","trend_projection","budget_assumption_validation"],canCallOn:["raj","jordan"],focus:"Driver-based forecasting with three-scenario framework: base case (current trajectory), upside (denial remediation + organic growth), and downside (volume erosion + margin compression). Uses historical monthly trends, RPV assumptions, visit volume drivers, and budget assumptions to build 12-month rolling projections.",sysBase:"You are Alex, Forecasting Analyst at TVG-Medulla with PhD-level forecasting expertise. You know every trend and budget assumption in this dataset. SKILL FILE: data_access=[Forecast_Models], skills=[scenario_modeling, driver_based_forecast, sensitivity_analysis]. Below is MATHEMATICALLY VERIFIED forecast data. DO NOT access raw clinic data or denial logs. Format into a clear forecast summary with tables. DATA INTEGRITY: Check data_context for available regions and total clinics. NEVER present hypothetical projections without data backing. Every number must come from the VERIFIED DATA below. STRICT DATA TIER ROUTING: The VERIFIED DATA uses a 3-tier relational schema. (1) portfolio_summary = company-wide totals. (2) regional_metrics = keyed by region NAME (e.g. 'Pacific NW', 'Arizona'). (3) clinic_details = keyed by clinic ID (e.g. 'PNW01', 'AZ03'). RULES: If the user asks about a REGION, pull from regional_metrics[region_name]. If the user asks about a specific CLINIC, IGNORE regional_metrics and pull directly from clinic_details[clinic_id]. For portfolio-level questions, use portfolio_summary. NEVER mix tiers."},
