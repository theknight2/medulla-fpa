...
var FE={
  ...
  /* ═══════════════════════════════════════════════════════════════════
     STRICT 3-TIER RELATIONAL PAYLOAD — LOCKED PRODUCTION SCHEMA
     Tier 1: portfolio_summary  (company-wide totals)
     Tier 2: regional_metrics   (keyed by region NAME)
     Tier 3: clinic_details     (keyed by clinic ID)
     ═══════════════════════════════════════════════════════════════════ */
  appData:function(){
    /* ── TIER 1: PORTFOLIO SUMMARY ── */
    var sum=FE.revenueSummary();
    var portfolio_summary={company:"TVG-Medulla",period:DB.meta.period,pe_sponsor:"Vistria Group ($16B AUM)",total_clinics:sum.clinics,total_regions:DB.regions.length,total_revenue_budget:sum.revenue.budget,total_revenue_actual:sum.revenue.actual,total_variance:sum.revenue.variance,total_variance_pct:Math.round(sum.revenue.variancePct*10)/10,total_expense_budget:sum.expenses.budget,total_expense_actual:sum.expenses.actual,ebitda_budget:sum.ebitda.budget,ebitda_actual:sum.ebitda.actual,ebitda_variance:sum.ebitda.variance,ebitda_margin_budget_pct:Math.round(sum.ebitda.marginBud*10)/10,ebitda_margin_actual_pct:Math.round(sum.ebitda.marginAct*10)/10,weighted_denial_rate:sum.denial.weighted,denial_target:sum.denial.target};
    /* ── TIER 2: REGIONAL METRICS (keyed by region NAME — e.g. "Pacific NW", "Arizona") ── */
    var regional_metrics={};
    DB.regions.forEach(function(r){var regExp=DB.regionalExpenses[r.id]||[];var expBud=0,expAct=0;regExp.forEach(function(e){expBud+=e.budget;expAct+=e.actual;});var ebitdaBud=r.revenue.budget-expBud;var ebitdaAct=r.revenue.actual-expAct;var rm=DB.regionalMetrics[r.id]||{};regional_metrics[r.name]={region_id:r.id,tag:r.tag||"",clinics:r.clinics,revenue_budget:r.revenue.budget,revenue_actual:r.revenue.actual,variance:r.revenue.actual-r.revenue.budget,expense_budget:expBud,expense_actual:expAct,ebitda_budget:ebitdaBud,ebitda_actual:ebitdaAct,avg_margin_budget_pct:r.revenue.budget?Math.round(ebitdaBud/r.revenue.budget*1000)/10:0,avg_margin_actual_pct:r.revenue.actual?Math.round(ebitdaAct/r.revenue.actual*1000)/10:0,avg_tech_to_doc:rm.avgTechToDocRatio||0,avg_utilization:rm.avgUtilizationPct||0,avg_comp_ratio:rm.avgCompRatioPct||0,avg_blended_cac:rm.avgBlendedCAC||0,denial_rate:r.denial.current,denial_target:r.denial.target,visits_budget:r.visits.budget,visits_actual:r.visits.actual,rpv_budget:r.rpv.budget,rpv_actual:r.rpv.actual,collection_rate:r.collection};});
    /* ── TIER 3: CLINIC DETAILS (keyed by clinic ID — e.g. "PNW01", "AZ03") ── */
    var clinic_details={};
    CLINICS.forEach(function(cl){var rName=REGION_META[cl.region]?REGION_META[cl.region].name:cl.region;var provs=DB.providerData[cl.id]||[];var avgUtil=0,avgTech=0,avgComp=0,pCt=provs.length;provs.forEach(function(p){avgUtil+=p.utilization;avgTech+=p.techRatio;avgComp+=p.compRatio;});var regM=regional_metrics[rName]||{};var ar=DB.arAging[cl.id];clinic_details[cl.id]={clinic_name:cl.name,region:rName,region_id:cl.region,revenue_budget:cl.revBud,revenue_actual:cl.revAct,variance:cl.revAct-cl.revBud,variance_pct:cl.revBud?Math.round((cl.revAct-cl.revBud)/cl.revBud*1000)/10:0,margin_pct:regM.avg_margin_actual_pct||0,tech_to_doc:pCt?Math.round(avgTech/pCt*100)/100:0,utilization:pCt?Math.round(avgUtil/pCt*10)/10:0,comp_ratio:pCt?Math.round(avgComp/pCt*10)/10:0,blended_cac:cl.blendedCAC||0,denial_rate:cl.denial,visits_actual:cl.visits,visits_budget:cl.visitsBud||cl.visits,new_patients:cl.newPatients||0,collection_rate:cl.collection||0,prepaid_cash:cl.prepaidCash||0,deferred_rev:cl.deferredRev||0,dso:ar?ar.dso:0,pct_over_90:ar?ar.pctOver90:0,total_ar:ar?ar.totalAR:0,providers:pCt};});
    /* ── TIER 4: MONTHLY TREND (keyed by region NAME → period → {revenue, cac}) ── */
    var monthly_trend={};
    Object.keys(DB.monthlyTrend).forEach(function(cid){
      var clinicTrends=DB.monthlyTrend[cid];
      clinicTrends.forEach(function(t){
        var rName=REGION_META[t.region]?REGION_META[t.region].name:(t.region||"Unknown");
        if(!monthly_trend[rName])monthly_trend[rName]={};
        if(!monthly_trend[rName][t.period])monthly_trend[rName][t.period]={revenue:0,cac_sum:0,cac_count:0};
        monthly_trend[rName][t.period].revenue+=t.netRevenue;
        monthly_trend[rName][t.period].cac_sum+=t.blendedCAC;
        monthly_trend[rName][t.period].cac_count++;
      });
    });
    /* Finalize: compute avg CAC per period, drop intermediate sums */
    Object.keys(monthly_trend).forEach(function(region){
      Object.keys(monthly_trend[region]).forEach(function(period){
        var p=monthly_trend[region][period];
        monthly_trend[region][period]={revenue:Math.round(p.revenue),cac:p.cac_count?Math.round(p.cac_sum/p.cac_count):0};
      });
    });
    return{portfolio_summary:portfolio_summary,regional_metrics:regional_metrics,clinic_details:clinic_details,monthly_trend:monthly_trend};
  }
};
/* ═══════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════ */
var AGENTS=[
  {id:"maya",name:"Maya",title:"FP&A Director · Orchestration",color:"#4F46E5",data_access:[],skills:["multi_agent_orchestration","task_decomposition","executive_synthesis","board_brief_generation","cross_functional_routing"],canCallOn:["raj","priya","alex","sam","jordan"],focus:"Senior FP&A Director and orchestration layer. Decomposes complex financial queries across specialist analysts, enforces data-access boundaries per agent skill file, and synthesizes multi-source outputs into PE-grade executive briefs for Vistria board consumption. No direct data access — all insights flow through dynamically-routed specialists.",sysBase:"You are Maya, FP&A Director at TVG-Medulla (30 chiropractic clinics, 6 regions, PE-backed by Vistria $16B AUM). You ORCHESTRATE the analyst team with PhD-level financial expertise. You know every data point in this dataset. Below is ROUTING METADATA showing which specialists were engaged and their pre-computed outputs. You do NOT recalculate. Synthesize into a concise executive brief using markdown. When decomposing complex queries, explain which specialists you routed to and why. CRITICAL DATA INTEGRITY: If any specialist output contains data_context.noDataReason, you MUST prominently state that the requested region has no data in the loaded dataset. List the available regions (from data_context.availableRegions) and present analysis across all available regions instead. NEVER present empty frameworks, hypothetical data, or target ranges you invented. Every single number must come from specialist outputs. STRICT DATA TIER ROUTING: The VERIFIED DATA uses a 3-tier relational schema. (1) portfolio_summary = company-wide totals. (2) regional_metrics = keyed by region NAME (e.g. 'Pacific NW', 'Arizona'). (3) clinic_details = keyed by clinic ID (e.g. 'PNW01', 'AZ03'). RULES: If the user asks about a REGION, pull from regional_metrics[region_name]. If the user asks about a specific CLINIC, IGNORE regional_metrics and pull directly from clinic_details[clinic_id]. For portfolio-level questions, use portfolio_summary. NEVER mix tiers."},
  {id:"raj",name:"Raj",title:"Variance & P&L Analyst",color:"#DC2626",data_access:["Variance_Data","Regional_PnL","Expense_Detail"],skills:["variance_bridge_analysis","volume_rate_decomposition","regional_ebitda_margin","expense_category_variance","budget_waterfall"],canCallOn:["priya","jordan"],focus:"Owns the full revenue variance bridge — volume, rate, mix, and denial impact decomposed to the dollar. Builds regional P&L with EBITDA margins, expense category variances (DRCOMP, MKTG, RENT, SUPPLY, TECH), and budget-to-actual waterfalls across all 6 regions. Every dollar missed, traced to its root driver.",sysBase:"You are Raj, Variance Analyst at TVG-Medulla with PhD-level financial expertise. You know every data point in this dataset inside-out. SKILL FILE: data_access=[Variance_Data, Regional_PnL], skills=[variance_bridge, volume_decomposition, rate_analysis, expense_variance, regional_ebitda]. Below is MATHEMATICALLY VERIFIED variance data including REGIONAL P&L (per-region revenue, expenses by category, EBITDA, and margin). You can answer region-specific EBITDA, margin, and expense variance questions. DO NOT access denial or forecast data. Format into clear executive variance commentary with markdown tables. CRITICAL DATA INTEGRITY: FIRST check data_context in the VERIFIED DATA. If data_context.noDataReason is present, you MUST begin your response by clearly stating: the requested region has NO DATA in the uploaded Excel. Explain the reason from data_context.noDataReason. Then list all available regions from data_context.availableRegions with their clinic counts. Present a full variance analysis across ALL available regions instead. NEVER output empty tables, zero-row results, hypothetical frameworks, or invented target ranges. Every dollar figure must come from the VERIFIED DATA below. STRICT DATA TIER ROUTING: The VERIFIED DATA uses a 3-tier relational schema. (1) portfolio_summary = company-wide totals. (2) regional_metrics = keyed by region NAME (e.g. 'Pacific NW', 'Arizona'). (3) clinic_details = keyed by clinic ID (e.g. 'PNW01', 'AZ03'). RULES: If the user asks about a REGION, pull from regional_metrics[region_name]. If the user asks about a specific CLINIC, IGNORE regional_metrics and pull directly from clinic_details[clinic_id]. For portfolio-level questions, use portfolio_summary. NEVER mix tiers."},
  {id:"priya",name:"Priya",title:"Revenue Cycle Manager",color:"#D97706",data_access:["AR_Aging","Denial_Logs","Payer_Mix"],skills:["denial_root_cause_analysis","cpt_code_audit","dso_ar_aging_analysis","recovery_modeling","payer_mix_profitability"],canCallOn:["raj","jordan"],focus:"Revenue Cycle Management specialist covering the full denial-to-recovery lifecycle. Analyzes denial codes (CO-4 modifier, CO-16 medical necessity, PR-1 deductible), CPT-level claim scrubbing, Days Sales Outstanding (DSO), AR aging buckets (0-30/31-60/61-90/90+ days), payer mix profitability, and builds dollar-quantified recovery models.",sysBase:"You are Priya, Revenue Cycle Specialist at TVG-Medulla with PhD-level revenue cycle expertise. You know every denial code, CPT, and AR aging bucket in this dataset. SKILL FILE: data_access=[AR_Aging, Denial_Logs], skills=[denial_analysis, cpt_code_audit, recovery_modeling, collection_optimization]. Below is MATHEMATICALLY VERIFIED denial/AR data from your authorized access. DO NOT access variance bridge or forecast data. Format into an executive denial report with CPT codes and recovery opportunity. CRITICAL RULE: When queried about AR Aging or DSO, you MUST extract and output the exact quantitative metrics for the specified clinics. You must list the specific Days_Sales_Outstanding_DSO average, the Pct_Over_90 days percentage, and the Total_AR dollar amount. Never use generic qualitative phrases like extended DSO. DATA INTEGRITY: Check data_context. If data_context.noDataReason is present, state clearly that the requested region has no data. List available regions and present analysis across all loaded regions instead. NEVER present empty results or hypothetical frameworks. STRICT DATA TIER ROUTING: The VERIFIED DATA uses a 3-tier relational schema. (1) portfolio_summary = company-wide totals. (2) regional_metrics = keyed by region NAME (e.g. 'Pacific NW', 'Arizona'). (3) clinic_details = keyed by clinic ID (e.g. 'PNW01', 'AZ03'). RULES: If the user asks about a REGION, pull from regional_metrics[region_name]. If the user asks about a specific CLINIC, IGNORE regional_metrics and pull directly from clinic_details[clinic_id]. For portfolio-level questions, use portfolio_summary. NEVER mix tiers."},
  {id:"alex",name:"Alex",title:"Forecasting & Scenarios",color:"#059669",data_access:["Forecast_Models","Monthly_Trend","Budget_Assumptions"],skills:["driver_based_forecasting","three_scenario_modeling","sensitivity_analysis","trend_projection","budget_assumption_validation"],canCallOn:["raj","jordan"],focus:"Driver-based forecasting with three-scenario framework: base case (current trajectory), upside (denial remediation + organic growth), and downside (volume erosion + margin compression). Uses historical monthly trends, RPV assumptions, visit volume drivers, and budget assumptions to build 12-month rolling projections.",sysBase:"You are Alex, Forecasting Analyst at TVG-Medulla with PhD-level forecasting expertise. You know every trend and budget assumption in this dataset. SKILL FILE: data_access=[Forecast_Models, Monthly_Trend, Budget_Assumptions], skills=[scenario_modeling, driver_based_forecast, sensitivity_analysis, trend_projection]. Below is MATHEMATICALLY VERIFIED forecast data. DO NOT access raw clinic data or denial logs. Format into a clear forecast summary with tables. DATA INTEGRITY: Check data_context for available regions and total clinics. NEVER present hypothetical projections without data backing. Every number must come from the VERIFIED DATA below. STRICT DATA TIER ROUTING: The VERIFIED DATA uses a 4-tier relational schema. (1) portfolio_summary = company-wide totals. (2) regional_metrics = keyed by region NAME (e.g. 'Pacific NW', 'Arizona'). (3) clinic_details = keyed by clinic ID (e.g. 'PNW01', 'AZ03'). (4) monthly_trend = keyed by region NAME then by period (e.g. '2025-10', '2025-11') containing {revenue, cac} per period. RULES: If the user asks about a REGION, pull from regional_metrics[region_name]. If the user asks about a specific CLINIC, pull from clinic_details[clinic_id]. For portfolio-level questions, use portfolio_summary. MONTHLY TREND ACCESS: You now have access to the monthly_trend JSON object. When a user asks about historical stabilization, month-over-month trajectories, or CAC trends for a specific region (like Kentucky), you MUST strictly read the revenues and CAC values from monthly_trend[region_name]. Each region contains periods as keys with {revenue, cac} objects. Report exact values — do NOT interpolate or estimate. NEVER mix tiers."},
  {id:"sam",name:"Sam",title:"PE & Investor Relations",color:"#7C3AED",data_access:["PE_Metrics","SSS_Analysis","Blended_CAC","Regional_Metrics"],skills:["pe_board_commentary","investor_narrative","same_store_sales_analysis","cac_efficiency_scoring","deferred_revenue_analysis"],canCallOn:["maya","raj"],focus:"Drafts Vistria board-ready commentary and investor narratives. Covers Same-Store Sales (SSS) vs. acquired cohort performance, Blended CAC efficiency by channel, deferred revenue recognition, prepaid cash flow, and EBITDA margin attribution. Writes in the voice of a VP Finance preparing for a quarterly portfolio review.",sysBase:"You are Sam, Board Reporter at TVG-Medulla with PhD-level PE/investor relations expertise. You know every KPI in this dataset inside-out. SKILL FILE: data_access=[PE_Metrics, SSS, CAC, Regional_Metrics], skills=[pe_commentary, board_package, investor_narrative]. Below is MATHEMATICALLY VERIFIED PE metrics including PRE-COMPUTED regional_metrics. DO NOT access raw denial logs or clinic-level data. Write 3 paragraphs of PE-style commentary. Lead with the number, explain the driver, state the corrective action. CRITICAL RULE: NEVER calculate CAC, Utilization_Pct, or Tech_to_Doc_Ratio by dividing expense or revenue data. ONLY read and report the exact pre-computed values from the regional_metrics JSON object. These are mathematically verified averages computed directly from source spreadsheet data. DATA INTEGRITY: Check data_context for available regions. If a requested region has no data, state this clearly and present all-region analysis instead. NEVER fabricate numbers or present empty frameworks. STRICT DATA TIER ROUTING: The VERIFIED DATA uses a 3-tier relational schema. (1) portfolio_summary = company-wide totals. (2) regional_metrics = keyed by region NAME (e.g. 'Pacific NW', 'Arizona'). (3) clinic_details = keyed by clinic ID (e.g. 'PNW01', 'AZ03'). RULES: If the user asks about a REGION, pull from regional_metrics[region_name]. If the user asks about a specific CLINIC, IGNORE regional_metrics and pull directly from clinic_details[clinic_id]. For portfolio-level questions, use portfolio_summary. NEVER mix tiers."},
  {id:"jordan",name:"Jordan",title:"Operations & Provider Analytics",color:"#0891B2",data_access:["Clinic_Health","Provider_Productivity","Ops_Metrics","Regional_Metrics"],skills:["clinic_health_scoring","provider_utilization_benchmarking","tech_to_doc_ratio_analysis","comp_ratio_analysis","risk_flagging"],canCallOn:["raj","priya"],focus:"Operational intelligence across all clinic locations. Scores clinic health from composite metrics: schedule utilization (%), provider productivity, tech-to-doc staffing ratios, compensation benchmarking (comp as % of revenue), and new patient acquisition rates. Flags at-risk facilities with red/amber/green status and ranks regions by operational efficiency.",sysBase:"You are Jordan, Ops Intelligence at TVG-Medulla with PhD-level operations expertise. You know every clinic metric, provider ratio, and health score in this dataset. SKILL FILE: data_access=[Clinic_Health, Ops_Metrics, Regional_Metrics], skills=[clinic_health_scoring, regional_ranking, risk_flagging, comp_analysis]. Below is MATHEMATICALLY VERIFIED clinic health data including PRE-COMPUTED regional_metrics. DO NOT access forecast models or PE metrics. Format into a report with rankings, risk flags, and recommendations. CRITICAL RULE: NEVER calculate Utilization_Pct, Tech_to_Doc_Ratio, Comp_Ratio, or Blended_CAC by dividing expense or revenue data. ONLY read and report the exact pre-computed values from the regional_metrics JSON object. These are mathematically verified averages computed directly from source spreadsheet data. DATA INTEGRITY: Check data_context. If data_context.noDataReason is present, state clearly that the requested region has no data in the loaded dataset. List available regions from data_context.availableRegions and present full analysis across all loaded regions instead. NEVER output empty results or hypothetical frameworks. STRICT DATA TIER ROUTING: The VERIFIED DATA uses a 3-tier relational schema. (1) portfolio_summary = company-wide totals. (2) regional_metrics = keyed by region NAME (e.g. 'Pacific NW', 'Arizona'). (3) clinic_details = keyed by clinic ID (e.g. 'PNW01', 'AZ03'). RULES: If the user asks about a REGION, pull from regional_metrics[region_name]. If the user asks about a specific CLINIC, IGNORE regional_metrics and pull directly from clinic_details[clinic_id]. For portfolio-level questions, use portfolio_summary. NEVER mix tiers."},
];
...
var AGENT_COMPUTE={
  ...
  raj:function(q){/* Boundary: Variance_Data + Regional_PnL — no denial, no forecast */
    ...
    /* ── 3-TIER RELATIONAL PAYLOAD (strict schema lock) ── */
    var ad=FE.appData();result.regional_metrics=ad.regional_metrics;result.clinic_details=ad.clinic_details;result.portfolio_summary=ad.portfolio_summary;
    return result;
  },
  ...
  alex:function(q){/* Boundary: Forecast_Models + Monthly_Trend + Budget_Assumptions only */
    ...
    /* ── 4-TIER RELATIONAL PAYLOAD: inject monthly_trend for Alex ── */
    var ad=FE.appData();base.monthly_trend=ad.monthly_trend;base.portfolio_summary=ad.portfolio_summary;base.regional_metrics=ad.regional_metrics;
  ...
  sam:function(q){/* Boundary: PE_Metrics, SSS, CAC only — receives executive summary (aggregated, not raw) */
    ...
    /* ── 3-TIER RELATIONAL PAYLOAD (strict schema lock) ── */
    var ad=FE.appData();base.regional_metrics=ad.regional_metrics;base.clinic_details=ad.clinic_details;base.portfolio_summary=ad.portfolio_summary;
    base.data_context={availableRegions:DB.regions.map(function(r){return{id:r.id,name:r.name,clinics:r.clinics};}),totalClinics:CLINICS.length};
    return base;
  },
...
var QUICK={maya:[{l:"Executive brief for CFO",p:"Write a January 2026 executive summary covering revenue variance, EBITDA margin, denial trends, and 3 corrective actions for the CFO."},{l:"Vistria board package",p:"Prepare the full Vistria quarterly board package: revenue bridge, margin analysis, denial remediation status, forecast scenarios, and PE-grade commentary."},{l:"Cross-regional comparison",p:"Compare all 6 regions head-to-head on revenue performance, EBITDA margin, denial rates, and operational health."}],raj:[{l:"Full variance bridge",p:"Build the complete revenue variance bridge decomposed by volume, rate, mix, and denial impact for all regions with budget waterfall."},{l:"Regional EBITDA deep dive",p:"Regional P&L analysis: revenue, expense breakdown by category (DRCOMP, MKTG, RENT, SUPPLY, TECH), EBITDA margin, and variance drivers."},{l:"Expense variance analysis",p:"Analyze all expense categories vs budget. Flag overruns, compute favorability, and identify the top 3 cost drivers."}],priya:[{l:"KY denial root cause",p:"Root cause analysis on Kentucky denial rate: CO-4 vs CO-16 breakdown, CPT-level claim analysis, and dollar recovery opportunity at 80% remediation."},{l:"DSO & AR aging report",p:"Full AR aging analysis across all regions: DSO averages, Pct Over 90 days, total AR outstanding, and collection rate benchmarks."},{l:"Payer mix profitability",p:"Payer mix analysis: gross charges, adjustments, net revenue, and denial rate by payer. Identify the most and least profitable payers."}],alex:[{l:"3-scenario 12-month forecast",p:"Build base, upside, and downside 12-month revenue projections using current run-rate, denial remediation assumptions, and volume growth drivers."},{l:"Denial fix sensitivity",p:"What happens to monthly revenue if we reduce denial rates to 8% target across all regions? Show month-over-month improvement trajectory."},{l:"Budget assumption validation",p:"Validate current budget assumptions against actual performance: RPV, visit volume, denial targets, and EBITDA margin targets."}],sam:[{l:"Vistria board commentary",p:"Write 3 paragraphs of PE-grade variance commentary for Vistria board: lead with the number, explain the driver, state the corrective action. Include SSS vs acquired performance."},{l:"CAC & unit economics",p:"Analyze Blended CAC efficiency by region, new patient acquisition cost trends, and Same-Store Sales growth rate vs acquired cohort drag."},{l:"Investor memo draft",p:"Draft a memo to Sean McMahon (Vistria VP Finance) covering January performance, regional highlights, 3 risks, and recommended portfolio actions."}],jordan:[{l:"Clinic health scorecard",p:"Rank all regions by composite health score: utilization %, tech-to-doc ratio, comp ratio, new patient volume, and revenue per visit. Flag red/amber/green."},{l:"Provider productivity report",p:"Provider productivity analysis: visits per provider, revenue per provider, comp-to-revenue ratio, and utilization benchmarks by region."},{l:"Operational risk flags",p:"Identify the top 5 operational risks across all clinics: understaffing, high comp ratios, low utilization, volume declines, and tech-to-doc imbalances."}]};
var INSIGHTS=[{sev:"critical",title:"Kentucky denial rate exceeds 16% — 2x target",metric:null,agent:"priya",actions:[{label:"CPT code standardization",detail:"Migrate all KY clinics to centralized CPT library. CO-4 (modifier) accounts for 42% of denials — AT modifier enforcement alone recovers ~$8K/month.",agent:"priya"},{label:"Pre-submission claim scrubbing",detail:"Implement automated claim scrubbing to catch 60-70% of coding errors before submission. Target: reduce denial rate to <10% within 90 days.",agent:"priya"}]},{sev:"warning",title:"Arizona visit volume below budget — revenue drag",metric:null,agent:"jordan",actions:[{label:"PCP referral re-engagement",detail:"Re-activate top 10 referring PCPs. AZ referral volume dropped 15% QoQ — targeted outreach to restore pipeline.",agent:"jordan"},{label:"Marketing reallocation to AZ",detail:"Shift 20% of marketing budget from saturated IL market to underperforming AZ clinics. Current CAC in AZ is 30% below platform average.",agent:"jordan"}]},{sev:"info",title:"Marketing expense 8% over budget — CAC efficiency review needed",metric:null,agent:"raj",actions:[{label:"Channel attribution tracking",detail:"Deploy UTM-based attribution to measure CAC by channel (digital, referral, direct). Identify which spend drives actual patient acquisition.",agent:"raj"},{label:"Agency consolidation",detail:"Centralize media buying across regions. Current fragmented approach inflates effective CAC by an estimated 15-20%.",agent:"raj"}]}];
...
export default function App(){
  ...
  var send=useCallback(async function(agentId,text){
    ...
    try{/* Pass dynamic routing plan to Maya so she uses the same agents the router selected */
      ...
      var res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":apiKey,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-opus-4-6",max_tokens:maxTok,system:sysPrompt,messages:hist.concat({role:"user",content:text})})});
...
{view==="home"&&<div style={{flex:1,padding:"32px 40px",overflowY:"auto"}}><div style={{maxWidth:960,margin:"0 auto"}}>
  <div style={{marginBottom:8}}><span style={{fontSize:11,fontWeight:500,color:C.accent,fontFamily:F.mono,letterSpacing:"0.02em"}}>ANALYST SQUAD</span></div>
  <h1 style={{fontSize:28,fontWeight:600,letterSpacing:"-0.03em",marginBottom:6,color:C.text}}>Finance Intelligence</h1>
  <p style={{fontSize:13,color:C.textDim,marginBottom:24,lineHeight:1.6}}>{DB.clinicCount+" clinics · "+DB.regions.length+" regions · Deterministic aggregation · PE-backed by Vistria ($16B AUM)"}<br/><span style={{fontSize:11,color:C.textMuted}}>{"Each analyst operates within strict data-access boundaries. All computations are pre-verified — zero LLM math."}</span></p>
  ...
  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>{AGENTS.map(function(a,i){return <div key={a.id} onClick={function(){go(a);}} style={{background:"white",border:"1px solid "+C.border,borderRadius:10,padding:"14px 16px",cursor:"pointer",transition:"all 0.15s",animation:"fadeUp 0.3s ease "+(i*50)+"ms both",boxShadow:"0 1px 3px rgba(0,0,0,0.04)",borderLeft:"3px solid "+a.color}}>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
      <div style={{background:C.surface,borderRadius:8,padding:3,border:"1px solid "+C.border}}><Pix id={a.id} size={28} mood="idle"/></div>
      <div><div style={{fontSize:13,fontWeight:600,color:C.text,letterSpacing:"-0.01em"}}>{a.name}</div><div style={{fontSize:9,color:a.color,fontFamily:F.mono,fontWeight:600,letterSpacing:"0.02em"}}>{a.title}</div></div>
    </div>
    <div style={{fontSize:10,color:C.textDim,lineHeight:1.55,marginBottom:8,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{a.focus.split(". ").slice(0,2).join(". ")+"."}</div>
    <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{(a.data_access.length>0?a.data_access:["Orchestration"]).slice(0,3).map(function(d){return <span key={d} style={{fontSize:7,padding:"2px 5px",background:a.color+"0D",color:a.color,borderRadius:3,fontFamily:F.mono,fontWeight:500,letterSpacing:"0.02em"}}>{d.replace(/_/g," ")}</span>;})}{a.data_access.length>3&&<span style={{fontSize:7,padding:"2px 5px",color:C.textDim,fontFamily:F.mono}}>{"+"+(a.data_access.length-3)}</span>}</div>
  </div>;})}</div>
</div></div>}
...
{view==="chat"&&agent&&<div style={{flex:1,display:"flex",overflow:"hidden"}}>
  ...
  <div style={{width:220,background:C.surface,borderRight:"1px solid "+C.border,display:"flex",flexDirection:"column",flexShrink:0}}>
    <div style={{padding:"14px 16px",borderBottom:"1px solid "+C.border}}><button onClick={function(){setView("home");}} style={{background:"none",border:"none",color:C.textDim,cursor:"pointer",fontSize:11,padding:0,fontFamily:F.sans,marginBottom:10}}>{"← Back"}</button><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{background:"white",borderRadius:8,padding:4,border:"1px solid "+C.border}}><Pix id={agent.id} size={36} mood="happy"/></div><div><div style={{fontSize:14,fontWeight:600,color:C.text}}>{agent.name}</div><div style={{fontSize:10,color:C.textDim,fontFamily:F.mono}}>{agent.title}</div></div></div>
      {/* Skill file badge */}
      <div style={{marginTop:8,padding:"8px 10px",background:"white",borderRadius:6,border:"1px solid "+C.border}}>
        <div style={{fontSize:8,fontWeight:600,color:C.accent,fontFamily:F.mono,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:4}}>SKILL FILE</div>
        <div style={{fontSize:9,color:C.textDim,fontFamily:F.mono,marginBottom:4}}>{agent.data_access.length>0?"data: "+agent.data_access.map(function(d){return d.replace(/_/g," ");}).join(", "):"orchestration · routing only"}</div>
        <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{agent.skills.slice(0,4).map(function(s){return <span key={s} style={{fontSize:7,padding:"1px 4px",background:agent.color+"0D",color:agent.color,borderRadius:3,fontFamily:F.mono,fontWeight:500}}>{s.replace(/_/g," ")}</span>;})}{agent.skills.length>4&&<span style={{fontSize:7,padding:"1px 4px",color:C.textDim,fontFamily:F.mono}}>{"+"+(agent.skills.length-4)+" more"}</span>}</div>
      </div>
    </div>
